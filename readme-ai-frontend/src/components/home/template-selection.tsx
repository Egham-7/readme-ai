...
const TemplatePreviewContent = ({
  ...
  <>
    <Tabs defaultValue="preview" className="w-full bg-muted/50 p-4 rounded-lg">
      <TabsList className="grid w-full grid-cols-2 bg-background">
        <TabsTrigger value="preview">Preview</TabsTrigger>
        <TabsTrigger value="code">Markdown</TabsTrigger>
      </TabsList>
      <TabsContent value="preview">
        <div className="prose prose-sm dark:prose-invert max-w-none bg-background border rounded-md p-4 h-[50vh] overflow-y-auto">
      ...
      <TabsContent value="code">
        <Textarea
          ...
          readOnly
          className="w-full h-[50vh] font-mono text-sm bg-background"
        />
      </TabsContent>
    </Tabs>
    <Button
...
>(({ name, onClick }, ref) => (
  <Button
    ref={ref}
    variant="outline"
    className="w-full h-24 flex flex-col items-center justify-center hover:bg-accent hover:text-accent-foreground"
    onClick={onClick}
    type="button"
  >
    <span className="font-bold">{name}</span>
    <span className="text-sm text-muted-foreground">Preview template</span>
  </Button>
));
          ...
          <Drawer key={template.id}>
            ...
            <DrawerContent className="bg-background">
              <DrawerHeader className="bg-muted/30">
                <DrawerTitle>{template.name} Template</DrawerTitle>
                <DrawerDescription>
  ...
  return (
    <div className="container mx-auto py-8 space-y-8">
      <Tabs defaultValue="featured" className="space-y-6">
        <TabsList className="justify-start bg-muted/30">
        ...
        <TabsContent value="community" className="space-y-6">
          <div className="flex justify-between items-center">
            <h3 className="text-xl font-semibold">Community Templates</h3>
